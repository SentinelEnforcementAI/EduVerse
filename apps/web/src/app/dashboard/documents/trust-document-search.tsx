...
// Conversational search across the whole trust repository (spec 5.9): the
// director types what they are looking for in plain language, and Watch matches
// on what a document says and the themes it covers, across every school, with
// the matched themes and a snippet so each result is explained.
export function TrustDocumentSearch() {
  const [input, setInput] = useState("");
  const [query, setQuery] = useState("");

  const search = api.documents.trustSearch.useQuery(
    { query },
      ...
      >
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Ask the repository — e.g. online safety, young carer support, information sharing thresholds"
            ...
            <>
              ...
              <ul className="mt-3 flex flex-col gap-2">
                {search.data?.hits.map((hit) => (
                  <li key={hit.id}>
                    <Link
                      href={`/dashboard/school/${hit.schoolId}/documents/${hit.id}`}
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {hit.schoolName} · {hit.type} ·{" "}
                          {hit.scope === "CASE" ? "Case" : "Vault"}
                        </span>
                      </div>
